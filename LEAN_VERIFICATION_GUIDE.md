...
论文第 6 节中的第一轮与第二轮结论属于同一次审计的历史记录，后续同步主线时不得用新的运行结果覆盖或改写：第一轮始终是上述 7 项未通过，第二轮始终是相同 20 个命题全部通过。论文记录的原始桥接运行包含 14 个 pytest 测试；本分支为 Windows 与 macOS 日志换行和尾随空白的稳定复现增加了 1 个回归测试，因此当前桥接显示 15 个测试。测试数的增加不改变命题集合、第一轮失败项、整改原因或第二轮业务结论。
...
### 11.1 同步主线时如何保持论文结论可比

同步主线、解决冲突或扩展分类功能时，按下面四层分别处理，不要把“历史结论”和“当前复验”混成一张会随代码变化的表：

1. 历史层固定保留第一轮 13/20 通过、七个失败 ID、各自根因和最小修复；
2. 规范层保持 20 个命题 ID、Lean 定义、Python 镜像判断和 20 个单点反例不变；
3. 实现层允许主线采用新的等价接口，但必须继续产生相同的可观测语义。例如当前分类流程可以从 `label_config` 取得 `custom_label_to_code`，再转换为相同的 `{label, code}` codec 并随模型持久化；这属于 L03 修复的接口适配，不是放宽 L03；
4. 证据层必须在合并提交之后重新生成 `production_trace.json`、两个生产报告和 `CurrentRun.lean`，并逐项核对 20/20 通过、1608/403 划分、单次拟合、状态摘要相等、两项 codec、身份错配拒绝和共享注册表无变更。

若主线实现已经用不同代码达到同一语义，应在提交说明中记录等价关系，并同时运行主项目 Python 3.9 的已安装 wheel 回归测试和本目录 Python 3.12 的完整桥接。只有当二者都通过时，才能把新的运行结果作为“当前复验”附加到分支；不得回填或重写论文表 1 的历史单元格。
