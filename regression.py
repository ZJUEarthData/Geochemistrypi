...
class PolynomialRegression(RegressionWorkflowBase, BaseEstimator):
    ...
    df = pd.read_excel(r'/Users/parnanjandutta/Downloads/Data_classification.xlsx')
    a= df['CAO(WT%)'].values
    b= df['FEOT(WT%)'].values
    c= df['SIO2(WT%)'].values
    d= df['TIO2(WT%)'].values
    e= df['AL2O3(WT%)'].values
    f= df['CR2O3(WT%)'].values
    g= df['MGO(WT%)'].values
    h= df['MNO(WT%)'].values
    i= df['NA2O(WT%)'].values
    x= vars() [input ("Enter which data?")]
    y= vars() [input ("Enter which data?")]
