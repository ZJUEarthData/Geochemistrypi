...
def test_manifest_tracks_every_active_typer_command_and_option() -> None:
    ...
    actual = {
        ...
        **{
            name: sorted(
                option for parameter in child.params for option in parameter.opts
            )
...
def test_manifest_tracks_cli_modes_models_and_bundled_datasets() -> None:
    ...
    assert declarations["bundled_dataset_ids"] == sorted(
        value[0] for value in _BUILT_IN_DATASETS.values()
    )
...
def test_supported_manifest_entries_have_evidence_and_known_gaps_are_visible() -> None:
    ...
    assert all(
        item["status"] == "verified" and item["evidence"]
        for item in capabilities
        if item["mcp_supported"]
    )
    assert all(
        not item["mcp_supported"]
        for item in capabilities
...
def test_repository_has_no_non_strict_xfail_escape_hatch() -> None:
    ...
    offenders = [
        ...
        and "strict=True" not in path.read_text(encoding="utf-8")
    ]
    assert offenders == []
