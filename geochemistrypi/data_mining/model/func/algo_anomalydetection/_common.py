...
def density_estimation(data: pd.DataFrame, labels: pd.Series, algorithm_name: str) -> None:
    """Generate a density estimation plot for anomaly detection."""
    # Scikit-learn anomaly detectors return 1 for inliers and -1 for outliers.
    normal_data = data.loc[labels == 1]
    anomaly_data = data.loc[labels == -1]
...
def scatter2d(data: pd.DataFrame, labels: pd.Series, algorithm_name: str) -> None:
    """
