...
def _build_classification_payload(target_col: str, model_name: str, num_classes: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "label_mapping": {
            ...
            "labels": [f"Level_{idx}" for idx in range(1, num_classes + 1)],
        },
...
def dash_pipeline(requests_pathname_prefix: str = "/dash/") -> dash.Dash:
    ...
    user_option = [{"label": "User's Uploaded Data", "value": "user_data"}] if (os.path.exists(USER_CSV) or os.path.exists(USER_XLSX)) else []
    dataset_options = user_option + [
    ...
    app.layout = html.Div(
        [
            ...
            html.Div("3. Number of user-defined classes:"),
            dcc.Input(id="num-classes-input", type="number", min=2, step=1, value=4),
    ...
    @app.callback(
        ...
        State("num-classes-input", "value"),
        prevent_initial_call=True,
    )
    def train_model_via_api(n_clicks, target_col, model_name, num_classes):
        if n_clicks == 0:
            return ""
        if not target_col:
        ...
        try:
            payload = _build_classification_payload(target_col, model_name, num_classes)
