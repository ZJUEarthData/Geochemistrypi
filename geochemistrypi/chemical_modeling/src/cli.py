...
app = typer.Typer(
    help="A command line tool to compare fsolve and root methods for solving equations."
)


@app.command()
def fsolve(
    phi_ref: float = typer.Option(
        0.5, "--phi-ref", "-p", help="Initial guess for φ_ref"
    ),
    beta_sple: float = typer.Option(
        0.5, "--beta-sple", "-b", help="Initial guess for β_sple"
    ),
    beta_mix: float = typer.Option(
        2.0, "--beta-mix", "-m", help="Initial guess for β_mix"
    ),
) -> None:
    """
