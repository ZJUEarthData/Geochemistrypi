...
def equations(params: Tuple[float, float, float]) -> List[float]:
    ...
    f1 = (
        φ_ref * R_100_sp
        + (1 - φ_ref) * R_100_std * (95 / 100) ** β_sple
        - r_100_mix * (95 / 100) ** β_mix
    )
    f2 = (
        φ_ref * R_98_sp
        + (1 - φ_ref) * R_98_std * (95 / 98) ** β_sple
        - r_98_mix * (95 / 98) ** β_mix
    )
    f3 = (
        φ_ref * R_97_sp
        + (1 - φ_ref) * R_97_std * (95 / 97) ** β_sple
        - r_97_mix * (95 / 97) ** β_mix
    )
...
def solve_equations(
    initial_guess: Tuple[float, float, float] = (0.5, 0.5, 2.0)
) -> np.ndarray:
    """
