"""
溶解度（SCSS）主流程模块，集成多种算法与模型。
- 支持 Rubie、Ding、Blanchard、Hybrid Model 等主流算法
- 支持 CLI 自动调度与批量数据处理
- 依赖 func.algo_solubility 下的 dataset、emodels、rmodels、nn_model
"""
...
# from geochemistrypi.chemical_modeling.model.func.algo_solubility.nn_model import NN, NNP  # 如需神经网络模型可解注

ROOT_DIR = Path(__file__).resolve().parent.parent
DATA_DIR = ROOT_DIR / "data" / "solubility"
SAMPLE_DATA_DIR = ROOT_DIR / "data" / "sample_data"
SAMPLE_DATA_FILE = SAMPLE_DATA_DIR / "s_sample.xlsx"


def get_sample_data_path() -> Optional[Path]:
    """返回默认的 S 样本数据文件路径。"""
    return SAMPLE_DATA_FILE if SAMPLE_DATA_FILE.exists() else None


def list_available_solubility_files() -> List[Path]:
    """列出已复制到 data/solubility 目录中的所有 Excel 数据文件。"""
    if DATA_DIR.exists():
        return sorted(DATA_DIR.glob("*.xls*"))
    return []
