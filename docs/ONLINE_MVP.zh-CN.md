...
本文档用于说明 Geochemistryπ 第一个可运行的 Online 版本。当前目标是先完成“选择算法 → 上传 Excel → 执行计算 → 下载结果”的完整流程，暂不追求正式上线和最终页面设计。
...
确认工作表第一行包含正确的列名。已验证的一阶动力学方法必须包含：
